...
void inverse_matrix()
{
	...
	for (i = 0; i < n; i++){
		...
		for (j = 0; j < n; j++)
			if (j < n - 1)
				fprintf(fw, " %10f * x%d +", a[i][j], j + 1); // це виведення початкової матриці, лише для оформлення
			else 
				fprintf(fw, " %10f * x%d ", a[i][j], j + 1); 
		fprintf(fw, "= %f", got[i]);
	}
		fprintf(fw, "\n---------------------------------------------------------------------------\n");
	for (i = 0; i < n; i++)
	...
	for (i = 1; i < n; i++){
		...
		for (j = 0; j < i; j++)
			{
				for (v = 0; v < n; v++)
					fprintf(fw, "%10f", a[j][v]);
				fprintf (fw, " | %10f", got[j]);
				fputs("\n", fw);
			}
		for (k = 0; k < n - i; k++){
			...
			for (j = 0; j < n; j++){
				...
				if (j < i)
					a[i + k][j] = 0;
				fprintf(fw, "%10f", a[i + k][j]);
			}

			got[i + k] -= got[i - 1] * d;
			fprintf (fw, " | %10f", got[i + k]);
			if (d > 0)
	...
	for (i = 0; i < n; i++){
		fprintf(fw, "\n");
		for (j = 0; j < n; j++)
			fprintf(fw, " %10f ", a[i][j]); // це виведення початкової матриці, якщо у ній діагональне значення == 0 або == INF то ця матриця вироджена і оберненої не існує
	}
	fprintf(fw, "\n---------------------------------------------------------------------------");
	fprintf(fw, "\nInversed matrix :");
	for (i = 0; i < n; i++){
	...
	for (i = 0; i < n; i++){
		fprintf(fw, "\n");
		for (j = 0; j < n; j++)
			fprintf(fw, " %10f ", a[i][j]);
	}
	fprintf(fw, "\n-----------------------------------------");
	for (i = 0; i < n; i++){
		fprintf(fw, "\nx%d = %10f ", i, got[i]);
	}
	printf("/nYou will find results in file rezMat.txt");
	getchar();
}
